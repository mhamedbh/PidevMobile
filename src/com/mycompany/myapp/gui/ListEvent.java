...
public class ListEvent extends BaseForm {
 ...
 Form current;
    public ListEvent(Resources res ) throws IOException {
        ...
        getContentPane().setScrollVisible(false);
         ...
         List<Event> list = EventService.getInstance().affichageEvent();
       ...
       //tri button
        Button btnFilterByBudget = new Button("Trier");
        btnFilterByBudget.addActionListener(new ActionListener() {
              @Override
              public void actionPerformed(ActionEvent evt) {
                  // order list by budget ( mel sghir lel kbir )
                  Collections.sort(list,(e1,e2) -> Float.compare(e1.getBudget_eve(), e2.getBudget_eve()));
                  // vitesse mtaa el transition men page el page hatineh 0 bech fisaa tetaada l'animation 
                  setTransitionOutAnimator(CommonTransitions.createSlide(CommonTransitions.SLIDE_VERTICAL, true, Integer.parseInt("0")));
                  try {
                      // display listevent w naatihwa el list baad ma aamalna tri
                      new ListEventFiltred(res,list).show();
